...
def send_message(
    ...
    # Se manca il destinatario o la piattaforma non e' specificata in modo
    # univoco, apri la nuova dashboard cross-platform (WhatsApp / Telegram /
    # Discord / Instagram) con scansione completa e selezione multipla.
    open_panel = (
        not receiver
        or platform in ("", "dashboard", "panel", "ask", "choose")
        or params.get("open_dashboard") is True
    )
    if open_panel:
        try:
            from actions.send_dashboard import open_dashboard
            open_dashboard(initial_text=message_text, prefer_gui=True)
