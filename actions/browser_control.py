...
def _find_browser_executable(prog_id: str) -> tuple:
    ...
    JARVIS preferred-browser policy (Mark XXXIX-OR):
      1. Always try Firefox first.
      2. If Firefox is not available, fall back to Chrome.
      3. The OS "default browser" is intentionally IGNORED.
    """
    system  = platform.system()
    os_bins = _BROWSER_BINARIES.get(system, {})

    # 1) Firefox first
    for binary in os_bins.get("firefox", []):
        path = shutil.which(binary)
        if path:
            print(f"[Browser] 🦊 Using Firefox at: {path}")
            return "firefox", path, None, False
    # Firefox may also be reachable via Playwright's bundled engine.
    # Playwright's `firefox.launch()` without executable_path uses its
    # own download, so try that as a second Firefox option.
    print("[Browser] 🦊 Firefox binary not found - trying Playwright Firefox engine")

    # 2) Chrome fallback
    for binary in os_bins.get("chrome", []):
        path = shutil.which(binary)
        if path:
            print(f"[Browser] 🌐 Firefox unavailable, falling back to Chrome at: {path}")
            return "chromium", path, None, False

    # 3) Last resort: Playwright bundled engines. Prefer firefox engine
    # since user asked for Firefox; if that fails at launch time the
    # caller's try/except will fall back to bundled chromium.
    print("[Browser] 🦊 Falling back to Playwright bundled Firefox")
