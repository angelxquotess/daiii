...
# Alias che JARVIS / il modello potrebbero usare al posto di "volume_set".
# Tutti questi vengono ricondotti a "volume_set" quando il chiamante passa
# anche un valore numerico (es. {"action": "volume", "value": 50}).
_VOLUME_SET_ALIASES = {
    "volume", "set_volume", "setvolume", "volume_to", "volumeto",
    "imposta_volume", "set_audio", "audio", "audio_volume",
    "change_volume", "adjust_volume", "master_volume",
}

# Alcuni modelli inviano action="control" con una description.
# In questo caso useremo _detect_action per inferire l'azione vera.
_GENERIC_ACTIONS = {"control", "do", "execute", "run", "settings"}
...
def computer_settings(
    ...
    # ---- Normalizzazione alias volume ----
    # Se il modello chiama 'volume', 'set_volume', 'control' ecc.
    # con un valore numerico, trattalo come volume_set.
    if action in _VOLUME_SET_ALIASES:
        action = "volume_set"
    elif action in _GENERIC_ACTIONS:
        # Se ci e' un valore numerico, supponiamo sia volume_set
        try:
            if value is not None and str(value).strip() != "":
                int(value)
                action = "volume_set"
        except (TypeError, ValueError):
            # Non e' numerico: prova a inferire dalla description
            if description:
                detected = _detect_action(description)
                action = (detected.get("action", "") or "").lower().strip().replace(" ", "_").replace("-", "_")
                if action in _VOLUME_SET_ALIASES:
                    action = "volume_set"
                if value is None:
                    value = detected.get("value")
