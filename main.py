...
TOOL_DECLARATIONS = [
    ...
    },
    {
        "name": "send_message",
        "description": (
            "Apre la dashboard cross-platform per inviare messaggi. "
            "Se receiver e platform sono noti invia direttamente; "
            "se mancano apre una finestra dove l'utente sceglie una o "
            "piu' piattaforme (WhatsApp, Telegram, Discord, Instagram), "
            "fa la scansione COMPLETA di tutte le chat e mostra una "
            "lista multi-selezione di destinatari, poi invia il messaggio."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "receiver":     {"type": "STRING", "description": "Recipient contact name (opzionale: se vuoto apre la dashboard)"},
                "message_text": {"type": "STRING", "description": "The message to send"},
                "platform":     {"type": "STRING", "description": "whatsapp | telegram | discord | instagram | dashboard"}
            },
            "required": []
        }
    },
    ...
    },
    {
        ...
        "description": (
            "Controlla WhatsApp Web dentro la mini-finestra di JARVIS. "
            "TUTTO via VOCE — non esiste piu' una dashboard manuale. "
            "Comandi tipici: "
            "'jarvis apri whatsapp' (action=open: mostra la finestrella), "
            "'jarvis mostrami le chat non lette' (action=read_chats), "
            "'jarvis invia un messaggio a <nome> e digli <testo>' o "
            "'jarvis manda un whatsapp a <nome>: <testo>' o "
            "'jarvis scrivi a <nome> <testo>' "
            "(action=send_message, recipient=<nome>, message=<testo>). "
            "Il nome puo' essere parziale o approssimativo (es. 'annala' "
            "trova 'Annalisa'): JARVIS fa fuzzy match su TUTTI i contatti, "
            "anche archiviati, usando la search di WhatsApp Web. "
            "'jarvis metti whatsapp a schermo intero' / 'massimizza whatsapp' "
            "(action=fullscreen: espande la finestrella a tutto il pannello "
            "centrale di JARVIS). "
            "'jarvis minimizza whatsapp' / 'rimetti whatsapp piccolo' / "
            "'torna alla finestra' (action=minimize: torna alla finestrella). "
            "'jarvis chiudi whatsapp' (action=close: nasconde la finestra). "
            "NON aprire mai il browser: tutto avviene nella finestrella."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action":    {"type": "STRING", "description": "open | read_chats | send_message | fullscreen | minimize | close"},
                "recipient": {"type": "STRING", "description": "Nome contatto (puo' essere parziale/approssimato — fuzzy match)"},
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        ...
        # Registra callback per nuovi messaggi WhatsApp in arrivo.
        # Quando l'overlay WA rileva una chat non letta nuova, chiama
        # questa funzione sul main thread Qt.
        try:
            self.ui.set_on_whatsapp_new_message(self._on_wa_new_message)
        except Exception as _e:
            print(f"[JARVIS] WA hook skipped: {_e}")

    def _on_wa_new_message(self, contact_name: str):
        """Nuovo messaggio WhatsApp ricevuto.

        Inietta nel Gemini Live un turno "sistema" che:
          1) ordina di annunciare vocalmente "Signore, nuovo messaggio
             da <contact_name>"
          2) memorizza nel contesto del modello che il prossimo
             "rispondi con <testo>" deve invocare whatsapp_control
             (action=send_message, recipient=<contact_name>, message=<testo>).
        """
        if not contact_name or not self._loop or not self.session:
            return
        # Sanitizza il nome
        cn = str(contact_name).replace("\n", " ").strip()
        ctx = (
            "[NUOVO MESSAGGIO WHATSAPP]\n"
            f"E' arrivato un nuovo messaggio da: {cn}\n"
            f"ISTRUZIONI:\n"
            f"1) Annuncia VOCALMENTE all'utente, in italiano: "
            f"\"Signore, nuovo messaggio da {cn}\". Una sola frase, breve.\n"
            ...
            f"IMMEDIATAMENTE chiamare il tool whatsapp_control con "
            f"action=\"send_message\", recipient=\"{cn}\" e message=<testo "
        ...
        try:
            ...
            print(f"[JARVIS] WA inject ctx err: {e}")
        ...
        try:
            if name == "open_app":
                ...
                try:
                    ...
                    self.ui.show_whatsapp_overlay(sub, rec, msg)
                    if sub == "read_chats":
                        result = "Sto leggendo le chat WhatsApp non lette, signore."
                    elif sub == "send_message":
                        if rec and msg:
                            result = f"Invio messaggio WhatsApp a {rec}: '{msg}'."
                        else:
                            result = "Mi servono il destinatario e il messaggio, signore."
